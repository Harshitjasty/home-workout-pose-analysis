...
def upload_video(request):
    ...
    try:
        if request.method == "POST":
            ...
            # Handle InMemoryUploadedFile vs TemporaryUploadedFile
            if hasattr(video, 'temporary_file_path'):
                video_file_path = video.temporary_file_path()
            else:
                # Create a temporary file
                fd, temp_video_path = tempfile.mkstemp(suffix='.mp4') # Use mkstemp for safety and path
            ...
            # Handle others data
            if exercise_type in ["squat", "bicep_curl"]:
...
from detection.realtime import VideoCamera, gen

@api_view(["GET"])
def video_feed(request):
    exercise_type = request.GET.get("type", "squat")
    return StreamingHttpResponse(gen(VideoCamera(exercise_type)),
